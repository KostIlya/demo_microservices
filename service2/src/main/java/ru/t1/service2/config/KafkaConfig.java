...
public class KafkaConfig {
    ...
    @Bean
    public ProducerFactory<String, TransactionResultEvent> transactionResultEventProducerFactory() {
        Map<String, Object> config = new HashMap<>();
        config.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG,
                environment.getProperty("spring.kafka.producer.bootstrap-servers"));
        config.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
    ...
    @Bean
    public KafkaTemplate<String, TransactionResultEvent> transactionResultEventKafkaTemplate() {
        return new KafkaTemplate<>(transactionResultEventProducerFactory());
    }

    @Bean
    NewTopic createT1DemoTransactionAccept() {
        return TopicBuilder.name(environment.getProperty("spring.kafka.topic.demo-transaction-accept"))
                .build();
    }
    @Bean
